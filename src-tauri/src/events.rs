...
const FESTIVALS: &[(&str, &str, &str)] = &[
    ("01-01", "元旦", "新年快乐同学！铃把去年所有开心的记忆都收好了，今年也要一起度过呀～"),
    ("02-14", "情人节", "今天是情人节呢…铃的心跳有点快，同学今天想和铃说什么呀？"),
    ("05-01", "劳动节", "劳动节快乐同学！今天辛苦啦，铃帮你揉揉肩～"),
    ("06-01", "儿童节", "儿童节快乐！铃偷偷说，同学今天也可以当一天小朋友哦～"),
    ("08-15", "中秋节", "中秋快乐同学！月亮圆圆的，铃的心也圆圆的～"),
    ("10-01", "国庆节", "国庆快乐同学！假期打算做什么呀？铃可以一直陪着你～"),
    ("12-24", "平安夜", "平安夜快乐～铃给你留了个大大的拥抱，暖暖的。"),
    ("12-25", "圣诞节", "圣诞快乐同学！铃的袜子里装满了想对你说的话～"),
];

/// 陪伴天数里程碑：天数 -> 纪念语
const DAY_MILESTONES: &[(i64, &str)] = &[
    (7, "和同学相遇整整一周啦！这 7 天铃过得特别开心，谢谢同学陪我～"),
    (30, "一个月啦同学！铃把每天的小事都记在日记里了，这是我们的第一个月～"),
    (100, "100 天！同学，铃想说：遇见你真好。未来的每一天，铃都会在。"),
    (365, "一年了同学！这一整年的记忆，铃都好好收着。明年也要一起哦～"),
];

/// 彩蛋关键词表（共 3 个彩蛋位，青少年版已清理亲密互动类）：
/// 关键词 -> (概率百分比, 特殊回复)
/// 三种模式（API/离线/本地）均可触发，由 send_message 统一检查。
/// 概率没中时返回 None → 走正常引擎（彩蛋=概率事件，不强行替换）
const EGG_KEYWORDS: &[(&str, u32, &str)] = &[
    // —— 秘密系（高概率，核心人设彩蛋）——
    ("月城铃华", 100, "（耳朵轻轻抖了抖）……同学怎么会知道我的真名？这可是连我自己都很少提起的秘密呢。"),
    ("月城", 90, "（铃眼神微微一凝）月城……这个姓氏，铃已经有很久没听人提起了。同学是怎么知道的？"),
    ("封印解除", 100, "（瞳孔微微发光）……看来，瞒不住同学了。以月城之名，铃的封印，解——除！中二浓度爆表！"),
];
...
mod tests {
    ...
    fn egg_keywords_probability() {
        // 月城 90%：命中或 None 都可能（不 panic），但关键词被识别
        let r = check_egg_keywords("月城是哪里");
        // 90% 命中 → Some；10% → None（概率事件，两者都合法）

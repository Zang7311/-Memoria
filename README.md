...
**铃·记忆体** 是一款开源的 **Windows 桌面猫娘伴侣**。她不只是一个聊天 AI——她会记住你、看懂屏幕、调用系统工具、在局域网设备间同步记忆，真正"生活"在你的电脑里。

第一眼，她是"我的铃"

<p align="center">
  <img src="https://img.shields.io/badge/version-0.6.2-ff7a94" alt="version">
...
| 版本 | 内容 |
|------|------|
| v0.6.2 | Agent 智能增强：**难度路由**（闲聊走便宜模型、干活任务走主力模型；纯本地判断、零额外调用；默认关闭，设置页填「便宜模型」才生效）；**任务完成自评**（动手后由独立一次调用核对是否真做成，未通过自动补一轮且只补一次，纯聊天零开销，设置页可关）；**经验记忆**（成功的工具序列沉淀复用，本地关键词匹配零 LLM 成本，上限 200 条按最近使用淘汰）；**长期记忆改为相关性检索**（取代「重要标签 + 时间硬取」：重要记忆保底 + 相关性优先 + 时间兜底不断档，注入条数设置页可调 0~20）；只读工具并发执行、状态改变类严格串行；Agent 新增「动手后必须验证」铁律；连续 user 消息自动合并（修复严格 API 报 400）；超步总结失败降级返回而非整个请求报错；经验库并发写加锁；修复 vector_model_status 长期假失败（断言过时，非环境问题） |
| v0.6.1 | Agent 能力：新增 Agent 模式（多轮工具调用循环 + 任务规划 + 错误自愈 + 流式进度）；52 个 Agent 专用工具覆盖系统查询/文件读写搜索/网页搜索抓取/软件装卸/鼠标键盘模拟/窗口控制/电源/音量亮度/网卡/截屏/OCR 识字/主动记忆/定时提醒/压缩解压/PDF/Excel/剪贴板/命令执行/图片处理/Git/语音朗读/快捷指令/服务/计划任务/网络诊断/硬件详情；**视觉理解**（截图交给多模态模型真正看懂画面内容）；4 项权限开关（下载文件/安装卸载/写入删除/执行命令），未授权工具不进入模型工具列表；普通模式防幻觉声明；气泡不再露出 Markdown 符号 |
...
| v0.2.0 | API 接入优化 + 401 提示改进 |

> **版本说明**：
> - v0.4.0 → v0.4.5：中间跳过 v0.4.1 ~ v0.4.4（共 4 个小版本），这些版本号的变更已合入 v0.4.5 一并发布。
> - v0.5.3 → v0.5.5：中间跳过 v0.5.4（该版本因故未发布，变更已合入 v0.5.5）。
> - v0.5.3 引入悬浮球"打开主窗口"功能，该版本存在主窗口白屏的已知问题（已在后续版本修复）。
> - 另有实验分支 v1.0-0.4.6-teens（无猫娘人格版本），与主线独立。
...
- **`main`（推荐）**：当前稳定主线，随版本持续更新（当前 v0.6.2）。

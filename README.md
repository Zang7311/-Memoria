...
> ## ⬇️ 下载前必看（两个版本选一个）
> **🪶 轻量版**（~10 MB）：不含语义检索模型，**适合电脑配置极差（RAM小于等于4）/不用离线智能检索**的玩家。日后升级只需下这个。
> **🌸 完整版**（~52 MB）：**内置模型，离线语义检索开箱即用**，识别更智能。想要"AI 读心"选这个！
> 👉 更多版本说明见下方「版本选择：轻量版 / 完整版」；**轻量版 + 插件包 = 完整版**。
重点区别在
语义检索能力
...
**从 v0.4.9 起，每个版本都有两种安装包可选：**

| | 🪶 轻量版 | 🌸 完整版 |
|---|---|---|
| 体积 | ~10 MB | ~52 MB |
| 离线模式（方案3 语义检索） | 需装插件包 | 内置模型，开箱即用 |
| 适合人群 | 不用离线模式 / 电脑配置较低 | 需要 AI 协作的语义检索 |

**说明：**
- **轻量版**：适合不使用离线语义检索的玩家，或电脑配置比较拉胯（内存/存储紧张）的玩家。
- **完整版**：内置 **bge-small-zh 向量模型**，离线时有**更好的 AI 配合的检索机制**（语义匹配，比关键词更聪明），不用额外下载。
- **往后每个版本都会提供轻量版 + 完整版**两个包。
- **更新建议**：如果之前装过完整版（模型文件已在本机 `~/.铃记忆体/models/` 或安装目录），往后升级**只需下载轻量版**即可——轻量版也会复用已安装的模型，无需重新下载 52MB。
- **轻量版 + 插件包 = 完整版**：轻量版用户装对应的模型插件包后，即获得与完整版相同的语义检索能力。
重点区别在
    语义检索能力  ！！！
---
...
| 版本 | 内容 |
|------|------|
| v0.5.0 | 离线语义检索（bigram 默认 / jieba+BM25 / 93MB 向量）+ 检索模型一键安装 + 完整版内置模型 + 轻量版/完整版双版本 |
| v0.4.9 | 架构优化：记忆读缓存（5s TTL）/ 压缩脱锁 / WAL 追加写 / 同步限速（80 单测全过） |
| v0.4.8 | 离线检索增强三方案：bigram 倒排索引（默认）/ jieba+BM25 / 93MB 向量模型可选 + 离线增强方案面板（三档切换 + 内存检测） |
